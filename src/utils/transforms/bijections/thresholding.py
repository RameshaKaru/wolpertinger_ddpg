...
class Threshold(Bijection):
    ...
    def inverse(self, z, one_hot: th.Tensor):
        ...
        z_no_op = z * one_hot
        zc = z.clamp(self.eps)
        z_op = (z + th.log1p(-th.exp(-zc))) * one_hot_not
...
if __name__ == '__main__':
    ...
    for k,v in instances.items():
        ...
        try:
            assert z[0].sum().round(decimals=2) == sol[k][0][0].sum().round(decimals=2)
            assert ldj.sum().round(decimals=2) == sol[k][1].sum().round(decimals=2)
            print('Test case {}: OK\n'.format(k))
        except:
